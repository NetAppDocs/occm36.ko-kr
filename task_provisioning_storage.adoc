...
summary: 볼륨 및 애그리게이트를 관리하여 Cloud Manager에서 Cloud Volumes ONTAP 시스템에 추가 NFS 및 CIFS 스토리지를 프로비저닝할 수 있습니다. 
---
= 스토리지 프로비저닝
:hardbreaks:
:allow-uri-read: 
...
Cloud Volumes ONTAP 시스템을 시작한 후 더 많은 스토리지가 필요한 경우 Cloud Manager에서 새 NFS 및 CIFS 볼륨을 프로비저닝할 수 있습니다.
...
.단계
. 작업 환경 페이지에서 볼륨을 프로비저닝할 Cloud Volumes ONTAP 시스템의 이름을 두 번 클릭합니다.
...
|===
. 필요한 경우 Usage Profile, Disk Type 및 Tiering Policy 페이지에서 스토리지 효율성 기능을 사용하도록 설정하고 디스크 유형을 선택한 다음 S3 계층화 정책을 편집할지 여부를 선택합니다.
+
자세한 내용은 다음을 참조하십시오.
...
. 호스트에서 LUN에 접속합니다.
+
자세한 내용은 를 참조하십시오 http://["Host Utilities 설명서"^] 를 참조하십시오.
