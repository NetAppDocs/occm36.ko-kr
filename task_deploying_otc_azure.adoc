...
| 태그 | 태그는 Azure 리소스에 대한 메타데이터입니다. Cloud Manager는 Cloud Volumes ONTAP 시스템과 시스템에 연결된 각 Azure 리소스에 태그를 추가합니다. 작업 환경을 만들 때 사용자 인터페이스에서 최대 4개의 태그를 추가할 수 있으며, 생성된 후에는 더 많은 태그를 추가할 수 있습니다. API는 작업 환경을 생성할 때 태그를 4개로 제한하지 않습니다. 태그에 대한 자세한 내용은 을 참조하십시오 https://["Microsoft Azure 문서: 태그를 사용하여 Azure 리소스를 구성합니다"^]. 
